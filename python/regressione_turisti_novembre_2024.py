...
def apply_regression(df_tourism):
    if all(col in df_tourism.columns for col in ["anno", "mese", "numero"]):
        ...
        # Escludere novembre 2024 dai dati di addestramento
        df_filtered = df_tourism[(df_tourism['anno'] >= 2021) & ~((df_tourism['anno'] == 2024) & (df_tourism['mese_num'] == 11))]
        ...
        # Previsione per novembre 2024
        X_pred = scaler_X.transform([[2024, 11]])
        y_pred_scaled = model.predict(X_pred)
        y_pred = scaler_y.inverse_transform([[y_pred_scaled[0]]])[0][0]

        valore_reale = df_tourism[(df_tourism['anno'] == 2024) & (df_tourism['mese_num'] == 11)]['numero'].values
        if valore_reale.size > 0:
            print(f"Numero reale di turisti per novembre 2024: {valore_reale[0]}")
        print(f"Numero stimato di turisti per novembre 2024: {y_pred:.2f}")
