...
df_filtered = df[(df['anno'] >= 2019) & ~((df['anno'] == 2024) & (df['mese'] > 10))]
...
# Creiamo il dato per Novembre 2024
X_pred = scaler_X.transform([[2024, 11, 145738]])
y_pred_scaled = model.predict(X_pred)
y_pred = scaler_y.inverse_transform(y_pred_scaled.reshape(-1, 1))[0, 0]

# Previsione
previsione_novembre_2024 = float(y_pred)  # Converti in float nativo di Python
...
# Descalare le previsioni per calcolare l'RMSE
...
# Risultati
print(f"Stima del prezzo per Novembre 2024: {previsione_novembre_2024:.2f}")
